...
class CodeGraphAnalyzer:
    ...
    def _analyze_file(self, file_path):
        ...
        self._merge_into_main_graph(
            graph_builder.graph, self._determine_node_size(len(content)))
        self._update_module_node_count(
            file_path, len(graph_builder.graph.nodes))
    ...
    def _prepare_nodes_and_edges(graph):
        nodes = [Node(id=n, label=n, size=data['size'])
                 for n, data in graph.nodes(data=True)]
        edges = [Edge(source=s, target=t, label=d.get('type', 'N/A'))
...
def main():
    ...
    repo_path = st.sidebar.text_input(
        "Repository Path", os.path.join(project_path, 'tests', 'test_repo'))
    ...
    selected_module = st.sidebar.selectbox(
        "Select Module", [module for module, _ in top_modules])
    ...
    with col2:
        if 'graph' in st.session_state:
            ...
            display_node_details(
                st.session_state['graph_analyzer'], selected_module)
    ...
    for node_id, node_data in module_graph:
        ...
        with contextlib.suppress(IOError):
            ...
            if file_content.strip():
                graph_builder = CodeUsageGraphBuilder(
                    file_content, file_path=node_data['file_path'])
                graph_builder.analyze_file()
                display_class_function_details(graph_builder, node_id)
                st.markdown(
                    f"**File Path**: [`{node_data.get('file_path', '')}`]({node_data.get('file_path', '')})")
...
def display_class_function_details(graph_builder, node_id):
    ...
    if 'expanded_classes' not in st.session_state:
        st.session_state['expanded_classes'] = {
            class_name: False for class_name in graph_builder.code_extractor.classes.keys()}
    if 'expanded_functions' not in st.session_state:
        st.session_state['expanded_functions'] = {
            func_name: False for func_name in graph_builder.code_extractor.functions.keys()}
    ...
    for class_name, class_info in graph_builder.code_extractor.classes.items():
        ...
        with col1:
            ...
            expand_button = st.button(
                '🚀 Ask LLM', key=f'ask_llm_class_{node_id}_{class_name}')
            ...
            if expand_button or expanded:
                ...
                additional_info = st.text_area(
                    "Additional information:", key=f'additional_info_{node_id}')

                if st.button("Submit to LLM", key=f'submit_llm_{node_id}'):
                    process_prompt(selected_prompt, additional_info,
                                   node_id, class_info.source_code)
    ...
    for func_name, func_info in graph_builder.code_extractor.functions.items():
        ...
        with col1:
            ...
            expand_button = st.button(
                '🚀 Ask LLM', key=f'ask_llm_func_{node_id}_{func_name}')
...
def process_prompt(prompt_template, additional_info, node_id, source_code=None):
    ...
    manager = RagManager('credentials.yaml',
                         'docstring_based_index', 'credentials.yaml')
    if prompt_template == 'Modify Code':  # modifyPrompt - context provided
        ...
        result = manager.search_for_vulnerabilities(
            additional_info, context=source_code)
